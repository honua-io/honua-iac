...
| `health_sources` | ALB readiness path and log group; `AWS/ApplicationELB` identifies provider metrics. `functional_check_path = null`: readiness is not a functional assertion. Native execution supplies a separate golden query and dedicated canary Prometheus source. |
