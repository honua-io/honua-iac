...
- Azure Functions cloud post-apply validation only exercises a configured deploy target when slot outputs are present. Non-slot runs still verify the deploy-plan endpoint contract, but they use the unknown-target `404` path instead of advertising a concrete rollout target that cannot be promoted.
