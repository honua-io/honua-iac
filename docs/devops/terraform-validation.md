...
- Azure ephemeral CI reuse works by omitting `--destroy-data` and restoring `${RUNNER_TEMP}/cache/azure-data-reuse.env`. AWS ephemeral CI reuse works by passing `--keep-data` and restoring `${RUNNER_TEMP}/cache/aws-data-reuse.env`.
