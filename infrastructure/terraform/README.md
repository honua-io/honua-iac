...
For local live integration runs, use the credentials helper script so you do not need to export variables manually each time.

```bash
cp scripts/tf-secrets.local.example.sh scripts/tf-secrets.local.sh
chmod 600 scripts/tf-secrets.local.sh
# edit scripts/tf-secrets.local.sh with your real values
source scripts/tf-secrets.local.sh
```

Sanity-check AWS auth before a live AWS run:
...
Serverless-only validation (Lambda path):
