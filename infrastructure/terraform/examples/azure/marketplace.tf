...
# marketplace/bundles/azure-aca.v1.json:
...
locals {
  ...
  deploy_contract = {
    ...
    endpoint       = module.honua.container_app_fqdn
    artifact_reference = {
    ...
    secret_refs = { for k, v in {
      db_connection    = module.honua.db_connection_secret_id
      admin_password   = module.honua.admin_password_secret_id
