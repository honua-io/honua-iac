...
# marketplace/bundles/aws-ecs.v1.json:
...
locals {
  ...
  install_net_https   = try(var.install.network.https_ingress_cidrs, null) != null ? var.install.network.https_ingress_cidrs : var.allow_https_ingress_cidrs
  install_net_http    = try(var.install.network.http_ingress_cidrs, null) != null ? var.install.network.http_ingress_cidrs : var.allow_http_ingress_cidrs
  ...
  install_contract = {
    ...
    network = merge(
      ...
      { for k, v in {
        ...
        http_ingress_cidrs  = local.install_net_http
        https_ingress_cidrs = local.install_net_https
      } : k => v if v != null },
    )
    storage = {
