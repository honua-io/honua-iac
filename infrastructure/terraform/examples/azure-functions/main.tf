...
module "honua" {
  ...
  scm_allowed_ip_cidrs                    = local.install_scm_allowed_ip_cidrs
  ...
  additional_env = {
    ...
    HostValidation__AllowedHosts__2 = "localhost"
    HostValidation__AllowedHosts__3 = "[::1]"
  }
}
...
locals {
  ...
  install_public_ingress_cidrs             = try(var.install.network.public_ingress_cidrs, null) != null ? var.install.network.public_ingress_cidrs : var.allowed_ip_cidrs
  install_public_network_access_enabled    = try(var.install.network.public_ingress_cidrs, null) != null ? length(var.install.network.public_ingress_cidrs) > 0 : var.public_network_access_enabled
  install_scm_allowed_ip_cidrs             = length(var.scm_allowed_ip_cidrs) > 0 ? var.scm_allowed_ip_cidrs : local.install_public_ingress_cidrs
  ...
  honua_url                                = local.install_public_network_access_enabled ? module.honua.function_app_url : null
