...
```hcl
module "honua" {
  source = "git::https://github.com/honua-io/honua-iac.git//infrastructure/terraform/modules/azure-functions?ref=v0.1.0"
  # ...inputs below...
}
```
...
When `deployment_slot_enabled = true`, the module also injects a `ControlPlane__DeployTargets__0__...` target definition into the app settings so the deployed Honua server can advertise and plan Azure Functions rollouts through `/api/v1/admin/deploy/*`. Non-slot deployments keep the admin deploy endpoint available but do not advertise a concrete Azure Functions rollout target.
