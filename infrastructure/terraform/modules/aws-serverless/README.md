...
Optional, **off by default**. When `enable_customcode_batch = true`, the module provisions a **SEPARATE, deliberately hardened** Batch family for running **untrusted user code** (the custom-code Python runtime). It parallels the GP substrate (Fargate-Spot scale-to-zero; a fixed pool of size tiers `customcode-s`/`m`/`l`/`xl` differing only by ephemeral storage) but **this family runs untrusted code, so it is locked down**:
...
- A **separate** compute environment + job queue (so untrusted jobs never share a queue with trusted GP jobs), `assignPublicIp = DISABLED`, private subnets. Reuses the Batch **service** role (orchestration only, no user-code trust). The Lambda role gets `batch:SubmitJob`/`TerminateJob`/`CancelJob` scoped to the **custom-code** queue + job definitions only.
- Optionally (`create_worker_customcode_repo = true`) a dedicated **`<name>-worker-customcode-python` ECR repository** — scan-on-push, KMS, image-count lifecycle cap, stable name.
