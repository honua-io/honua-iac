  ...
  value = {
    ...
    secret_store = {
      ...
      diagnostics = {
        ...
        diagnostic_setting_id      = var.key_vault_diagnostics_enabled && (trimspace(var.key_vault_diagnostics_workspace_id) != "" || var.log_analytics_enabled) ? azurerm_monitor_diagnostic_setting.key_vault[0].id : null
        log_analytics_workspace_id = var.key_vault_diagnostics_enabled && (trimspace(var.key_vault_diagnostics_workspace_id) != "" || var.log_analytics_enabled) ? local.key_vault_diagnostics_workspace_id : null
      }
    }
    secrets = {
      admin_password = {
