...
AWS_ROOT="$REPO_ROOT/infrastructure/terraform/examples/aws"
...
# The contract producer must never project a raw credential variable.
if grep -nE 'var\.(honua_admin_password|db_password|honua_connection_encryption_master_key|redis_connection_string|existing_db_connection_string|canary_header_value)' \
  "$AWS_ROOT/operator-contract.tf" >/dev/null 2>&1; then
  fail "operator-contract.tf references a raw credential variable"
else
  pass "operator-contract.tf references no raw credential variable"
fi
